...
if __name__ == '__main__':
    ...
    lock = RLock()  # Замок для ограничения доступа к общим ресурсам сразу несколькими процессами

    def genarate_proceses(link_list, base_url, tm, deep, max_links):
        '''
        Функция для генерации процессов с передачей им функции run и её аргументов
    ...
    def main(base_url, tm, deep, max_links):
        ...
        link_parser = Link_Parser(base_url, tm, deep)
        link_parser.get_links()  # Получение ссылок на странице

        start = datetime.now()

        genarate_proceses(link_parser.link_list, base_url, tm, deep, max_links)
    ...
    arguments.add_argument('-ml', type=int, default=1000, help='Max width of parsing, default == 2000')
